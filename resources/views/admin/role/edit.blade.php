...
@section('content')
        <form class="form-row" method="POST" action="{{ route('roles.update', $role->id) }}">
            @csrf
            @method('PUT')
            <div class="form-group col-12">
                <label for="name">Role Name (Permissions: {{ $role->permissions->pluck('name') }})</label>
                <input type="text" name="name" value="{{ $role->name }}" id="name" class="form-control" required>
            </div>
            <div class="form-group col-12">
